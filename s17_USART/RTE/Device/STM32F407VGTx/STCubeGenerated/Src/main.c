...
int main(void)
{
  ...
  while (1)
  {
		...
		/**************************************************/
		// HAL_UART_Transmit(&huart3, (uint8_t*)send, 6, 10);  
		
		// HAL_UART_Receive(&huart3, receive, data_size, timeout);
		
		for (int i=0; i<6; i++)
		{
			HAL_UART_Transmit(&huart3, &send[i], 1, 10);  
		}
		
		//for (int i=0; i<2; i++)
		{
			HAL_UART_Receive(&huart3, &receive, 1, 10);
		}
